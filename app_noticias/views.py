...
#Retorna sugestão de noticias, baseado na categoria da noticia atual
class NoticiaDetalhesView(DetailView):
    ...
    def get_context_data(self,**kwargs):
        context = super().get_context_data(**kwargs)
        context['sugestao'] = Noticia.objects.exclude(pk=self.object.id).filter(categoria=self.object.categoria).order_by('-id')[:5]
        return context
